...
class MainActivity : AppCompatActivity() {
    ...
    val catsListFragment get() = _catsListFragment
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if (id == R.id.item2) {
            ...
            item.isChecked = true
            Toast.makeText(applicationContext, "SQLite mode", LENGTH_SHORT).show()
        }
        if (id == R.id.item3) {
            viewModel.setMode(MODE_ROOM)
            viewModel.setModeDb(MODE_ROOM)
            item.isChecked = true
            Toast.makeText(applicationContext, "Room mode", LENGTH_SHORT).show()
        }
        return super.onOptionsItemSelected(item)
    }

    override fun onBackPressed() {
        if (currentFragment == catsListFragment) {
            finishAndRemoveTask()
